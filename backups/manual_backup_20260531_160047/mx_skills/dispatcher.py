...
SKILL_REGISTRY: dict[str, dict[str, Any]] = {
    ...
    "post-loan-analysis-report": {
        "entry": "_post_loan_placeholder",
        "description": "贷后分析页面结构化输出适配器（等待正式 post-loan 执行脚本接入）",
    },
}
...
async def execute_skill(
    ...
    **extra_kwargs,
    ...
    try:
        ...
        if entry_name == "_post_loan_placeholder":
            return {
                "status": "error",
                "error": (
                    "post-loan-analysis-report 后端尚未接入可执行的公开披露检索与 PDF 解析脚本；"
                    "当前不能据此判断企业是否有最新披露。"
                ),
                "skill": skill_name,
                "result": _serialize_result(_build_post_loan_placeholder(query)),
                "files": _list_output_files(output_path),
            }
