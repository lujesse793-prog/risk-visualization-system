...
SERVER_ALIASES = {
    ...
    "a-stock": "hexin-ifind-ds-stock-mcp",
    "fund": "hexin-ifind-ds-fund-mcp",
    "edb": "hexin-ifind-ds-edb-mcp",
    "macro": "hexin-ifind-ds-edb-mcp",
    "news": "hexin-ifind-ds-news-mcp",
    "search": "hexin-ifind-ds-news-mcp",
    "bond": "hexin-ifind-ds-bond-mcp",
    "global": "hexin-ifind-ds-global-stock-mcp",
    "global-stock": "hexin-ifind-ds-global-stock-mcp",
    "hk-us": "hexin-ifind-ds-global-stock-mcp",
...
class MCPConfig:
    ...
    def resolve_server_name(self, name: str) -> str:
        ...
        if normalized in self._config.get("mcpServers", {}):
            return normalized
        return SERVER_ALIASES.get(normalized, f"hexin-ifind-ds-{normalized}-mcp")

    def get_server(self, name: str) -> dict | None:
        """获取指定 MCP 服务器配置"""
        key = self.resolve_server_name(name)
    ...
    def list_server_infos(self) -> list[dict[str, Any]]:
        """列出前端可展示的 MCP 服务器信息，不暴露 Authorization"""
        infos = []
        for full_name, config in self._config.get("mcpServers", {}).items():
            short_name = full_name.replace("hexin-ifind-ds-", "").replace("-mcp", "")
            aliases = sorted([alias for alias, target in SERVER_ALIASES.items() if target == full_name])
            token = self.auth_token or config.get("headers", {}).get("Authorization", "")
            infos.append({
                "name": short_name,
                "full_name": full_name,
                "aliases": aliases,
...
class MCPClient:
    ...
    def __init__(self, server_name: str):
        ...
        self.token = self.config.auth_token or self.server.get("headers", {}).get("Authorization", "")
        if not self.token:
