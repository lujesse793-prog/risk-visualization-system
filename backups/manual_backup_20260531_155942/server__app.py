...
def _complete_task_from_result(task_id: str, result: dict):
    skill_result = result.get("result") if isinstance(result, dict) else None
...
def _run_post_loan_task_worker(task_id: str, skill_name: str, query: str, output_dir: str | None):
    ...
    for step_key, platform_key, action in platform_steps:
        ...
        time.sleep(0.8)
        if executor_missing:
            _update_platform_log(task_id, platform_key, "skipped", "后端公开披露检索执行器未接入。", "未发起真实平台检索。")
            _update_step(
                task_id,
                step_key,
                "skipped",
                "后端尚未接入该平台的真实检索执行器，已跳过该节点。",
                "未发起真实平台检索。",
                "post_loan_executor_not_configured",
                "已继续更新后续节点状态，并返回可渲染的无数据结构。",
            )
    ...
    for step_key in ["filter_announcements", "verify_subject", "parse_pdf", "extract_tables", "calculate_indicators", "generate_findings", "organize_evidence"]:
        ...
        _update_step(
            ...
            step_key,
            "skipped",
            label_map.get(step_key, "等待前置节点完成。"),
            "前置检索执行器未接入，无法执行该节点。",
            "post_loan_executor_not_configured",
            "未编造资料、指标或风险提示。",
        )
